...
[MCP](https://modelcontextprotocol.io) server exposing the Checklyhq SDK as
two agent tools — `checklyhq_list` and `checklyhq_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/checklyhq-mcp)
make build

# 2. Provide credentials via the environment
export CHECKLYHQ_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user checklyhq \
  -- /absolute/path/to/checklyhq-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./checklyhq-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// checklyhq_list: first page of records
{ "entity": "check" }
{ "entity": "check", "query": { } }

// checklyhq_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/checklyhq-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user checklyhq \
     -- "$PWD"/dist/*/checklyhq-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `checklyhq_list` and `checklyhq_load` tools now appear
   in new sessions. Ask the agent to *"list check using checklyhq"*
   and it calls `checklyhq_list` with `{"entity":"check"}`.
...
```sh
export CHECKLYHQ_APIKEY=sk_live_xxx            # API key
export CHECKLYHQ_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `checklyhq_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `checklyhq_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CHECKLYHQ_APIKEY` | API key sent with every request. |
| `CHECKLYHQ_BASE` | Optional override of the API base URL. |
...
```sh
./checklyhq-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"checklyhq_load","arguments":{"entity":"check","query":{"id":1}}}}'
```
