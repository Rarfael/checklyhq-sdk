...
from checklyhq_sdk.checklyhq_types import (
    Check,
    CheckLoadMatch,
    CheckListMatch,
    CheckCreateData,
...
class CheckEntity:
    ...
    def update(self, reqdata: CheckUpdateData, ctrl=None) -> Check:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> Check:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.Check().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
