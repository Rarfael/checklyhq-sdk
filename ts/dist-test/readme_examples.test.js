...
function isRunnable(code) {
    return (/new\s+ChecklyhqSDK\b/.test(code) ||
        /\bChecklyhqSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+ChecklyhqSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/ChecklyhqSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
