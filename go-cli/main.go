// Command Checklyhq-cli is a CLI + REPL for the Checklyhq SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CHECKLYHQ_APIKEY (api key), CHECKLYHQ_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CHECKLYHQ_APIKEY carries the API key and
	// CHECKLYHQ_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CHECKLYHQ_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CHECKLYHQ_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewChecklyhqSDK(opts)
