...
boru-driven command-line client **and** interactive REPL for the Checklyhq
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/checklyhq-cli)
make build
...
./checklyhq-cli list check
./checklyhq-cli load 1 check            # {id:1} shorthand
./checklyhq-cli load '{id:1}' check       # explicit match map
./checklyhq-cli update '{name:"x"}' check

# 5. Override the API base URL for a single call
CHECKLYHQ_BASE=https://api.example.com ./checklyhq-cli list check

# 6. No arguments -> interactive REPL
./checklyhq-cli
checklyhq> list check
checklyhq> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/checklyhq-cli
   ```
...
```sh
./checklyhq-cli load 1 check          # scalar shorthand for {id:1}
./checklyhq-cli load '{id:1}' check     # explicit match map
```
...
```sh
./checklyhq-cli update '{id:1,name:"new"}' check
```
...
Run with no arguments to open a REPL (prompt `checklyhq>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/checklyhq-cli`. |
...
(`./checklyhq-cli <expr>`) and interactively (the REPL), and expressions compose the
