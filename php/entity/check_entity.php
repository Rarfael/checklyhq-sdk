...
class CheckEntity
{
    ...
    {
        if ($args) {
            $this->_data = ChecklyhqHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = ChecklyhqHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws ChecklyhqError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ChecklyhqHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws ChecklyhqError on failure (item-5 convention).
     */
     ...
     *
     * @param CheckUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed CheckUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Check|array The updated Check as an assoc-array at the
     *   SDK boundary; throws ChecklyhqError on failure (item-5 convention).
     */
    ...
    /**
     * Remove an Check matching the given criteria.
     *
     * @param CheckRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; CheckRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Check|array The removed Check as an assoc-array at the
     *   SDK boundary; throws ChecklyhqError on failure (item-5 convention).
     */
