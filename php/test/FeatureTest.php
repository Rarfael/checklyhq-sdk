...
class FtCtrl extends ChecklyhqControl
{
...
class FtClient
{
    ...
    private ChecklyhqUtility $utility;

    public function __construct(ChecklyhqUtility $utility, array $options)
    {
    ...
    public function get_utility(): ChecklyhqUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ChecklyhqContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new ChecklyhqUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (ChecklyhqContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ChecklyhqConfig::make_feature($name);
    ...
    private static function buildUrl(ChecklyhqSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ChecklyhqError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ChecklyhqResponse($fetched);
        }
    ...
    private function fail(ChecklyhqContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ChecklyhqContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
