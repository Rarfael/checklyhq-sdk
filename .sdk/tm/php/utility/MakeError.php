...
class ChecklyhqMakeError
{
    public static function call(?ChecklyhqContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ChecklyhqContext([], null);
        }
        $op = $ctx->op ?? new ChecklyhqOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ChecklyhqResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ChecklyhqError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ChecklyhqError('', $msg, $ctx);
        ...
        if ($err instanceof ChecklyhqError) {
